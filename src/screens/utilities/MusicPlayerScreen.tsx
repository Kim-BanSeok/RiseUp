...
const MusicPlayerScreen = React.memo(() => {
  ...
  const loadSound = useCallback((track: Track): Promise<Sound> => {
    return new Promise((resolve, reject) => {
      if (soundRef.current) {
        soundRef.current.release();
      }

      const sound = new Sound(track.audioFile || 'alarm_gentle.mp3', Sound.MAIN_BUNDLE, (error) => {
        if (error) {
    ...
    try {
      ...
      sound.play((success) => {
        if (success) {
          ...
          console.log('재생 실패');
          showCustomAlert('재생 오류', '음악을 재생할 수 없습니다.');
        }
      });
      ...
      {showPlayer && currentTrack && (
        <View style={styles.miniPlayer}>
          ...
          <View style={styles.miniPlayerControls}>
            ...
            <TouchableOpacity onPress={stopPlayback} style={styles.controlButton}>
              <Text style={styles.controlIcon}>⏹</Text>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  tracksContainer: {
    paddingHorizontal: 20,
    paddingBottom: 200, // 탭과 미니플레이어 높이를 고려한 여백
  },
  trackItem: {
    flexDirection: 'row',
    alignItems: 'center',
