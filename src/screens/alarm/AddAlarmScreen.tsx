...
export default function AddAlarmScreen({ navigation }: any) {
  ...
  const { addAlarm, alarmGroups } = useAlarm();
  const [selectedGroupId, setSelectedGroupId] = useState(alarmGroups[0]?.id || 'personal');
  ...
  const saveAlarm = async () => {
    ...
    try {
      const success = await addAlarm(time, label.trim(), selectedDays || [], selectedSoundId, selectedGroupId);
          ...
          <View style={styles.section}>
            ...
            <View style={styles.groupSelectRow}>
              {alarmGroups.map(group => (
                <TouchableOpacity
                  key={group.id}
                  style={[styles.groupSelectButton, selectedGroupId === group.id && styles.groupSelectButtonActive]}
                  onPress={() => setSelectedGroupId(group.id)}
                >
                  <Text style={styles.groupSelectIcon}>{group.icon}</Text>
